...
const ChartLegendTooltipContent = (_a) => {
    var { activePoints, center, datum, dx = 0, dy = 0, flyoutHeight, flyoutWidth, height, labelComponent = React.createElement(ChartLegendTooltipLabel_1.ChartLegendTooltipLabel, null), legendComponent = React.createElement(ChartLegend_1.ChartLegend, null), legendData, patternScale, text, themeColor, 
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
    themeVariant, title, titleComponent = React.createElement(ChartLabel_1.ChartLabel, null), width, 
    // destructure last
    theme = (0, ChartUtils_1.getTheme)(themeColor) } = _a, rest = tslib_1.__rest(_a, ["activePoints", "center", "datum", "dx", "dy", "flyoutHeight", "flyoutWidth", "height", "labelComponent", "legendComponent", "legendData", "patternScale", "text", "themeColor", "themeVariant", "title", "titleComponent", "width", "theme"]);
    const pointerLength = theme && theme.tooltip ? victory_core_1.Helpers.evaluateProp(theme.tooltip.pointerLength) : 10;
    const legendProps = (0, ChartUtils_1.getLegendTooltipDataProps)(legendComponent.props);
    const visibleLegendData = (0, ChartUtils_1.getLegendTooltipVisibleData)({
        activePoints,
    ...
    const maxLegendDimensions = (0, ChartUtils_1.getLegendTooltipSize)({
        legendData: visibleLegendData,
        legendProps,
        text: (0, ChartUtils_1.getLegendTooltipVisibleText)({ activePoints, legendData, text }),
        theme
    });
    const minLegendDimensions = (0, ChartUtils_1.getLegendTooltipSize)({
        legendData: [{ name: '' }],
        legendProps,
        theme
    });
    ...
    const getLegendComponent = () => React.cloneElement(legendComponent, Object.assign({ data: (0, ChartUtils_1.getLegendTooltipVisibleData)({
            activePoints,
            colorScale: legendProps.colorScale,
            legendData,
            patternScale,
            ...
            theme
        }), labelComponent: getLabelComponent(), patternScale, standalone: false, theme, x: getX() + legendOffsetX + victory_core_1.Helpers.evaluateProp(dx), y: getY() + legendOffsetY + victory_core_1.Helpers.evaluateProp(dy) }, legendProps));
